-- E5.1 fix — Pass 3b was overwriting g instead of accumulating. Merge the two
-- generation reconciliation passes into a single CTE chain so the count is
-- correct.
...
BEGIN
  ...
  SELECT count(*)::integer INTO g FROM gen_dead_upd;

  -- Pass 3b: stale generation_status = generating with no active generate_items jobs
  -- Accumulate into g, don't overwrite.
  WITH stale_gen AS (
    UPDATE public.documents doc
    SET generation_status = 'failed',
        updated_at = now()
  ...
  SELECT g + count(*)::integer INTO g FROM stale_gen;
